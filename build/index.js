...
// Validator functions
const isValidBrowseDocsArgs = (args) => {
    return typeof args === 'object' &&
        args !== null &&
        typeof args.repository === 'string' &&
        (args.path === undefined || typeof args.path === 'string');
};
const isValidAddRepoMappingArgs = (args) => {
    return typeof args === 'object' &&
        args !== null &&
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (name === 'browse_docs') {
                if (!isValidBrowseDocsArgs(args)) {
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for browse_docs tool');
                }
                try {
            ...
            // Add repo mapping tool
                ...
                if (!isValidAddRepoMappingArgs(args)) {
                    throw new McpError(ErrorCode.InvalidParams, 'Invalid arguments for add_repo_mapping tool');
                }
                try {
